...
3. ECS deployment: builds the backend Docker image, pushes it to ECR, updates the ECS Fargate service to two tasks, waits for service stability, and verifies `/api/health` through the load balancer.

Terraform lives in `infra/terraform` and provisions the required unique S3 bucket with versioning, encryption, and public access blocked, plus ECR, ECS Fargate, and load-balancer resources.
